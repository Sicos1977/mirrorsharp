...
public class SlowUpdateHandlerTests {
    ...
    public async Task SlowUpdate_ProducesDiagnosticWithCustomTagUnnecessary_ForUnusedNamespace() {
        ...
        Assert.Contains(
            ...
            result.Diagnostics.Select(d => new { severity = d.Severity, isUnnecessary = d.Tags.Contains(WellKnownDiagnosticTags.Unnecessary, StringComparer.OrdinalIgnoreCase) }
            ).ToArray()
        );
    }
    ...
    public async Task SlowUpdate_Succeeds_ForValidVisualBasicCode() {
        var driver = MirrorSharpTestDriver.New(new MirrorSharpOptions().EnableVisualBasic(), LanguageNames.VisualBasic).SetText(@"
                Public Class C
                    Public Sub M()
                    End Sub
                End Class
...
#pragma warning disable RS2008 // Enable analyzer release tracking
        private static readonly DiagnosticDescriptor Descriptor = new("T01", "Test", "Test", "Test", DiagnosticSeverity.Warning, true);
