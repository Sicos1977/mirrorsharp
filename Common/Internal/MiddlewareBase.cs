...
internal abstract class MiddlewareBase {
    ...
    private ImmutableArray<ICommandHandler> CreateHandlersIndexedByCommandId() {
        ...
        foreach (var handler in CreateHandlers()) handlers[handler.CommandId - 'A'] = handler;
    ...
    protected async Task SlowTestStatusAsync(CancellationToken cancellationToken) {
        ...
        Task ExecuteAsync(char commandId, string command) {
            ...
            try {
                ...
                var asyncData = new AsyncData(bytes.AsMemory(0, byteCount), false, () => throw new Exception());
        ...
        foreach (var (commandId, commandText) in _options.StatusTestCommands) await ExecuteAsync(commandId, commandText).ConfigureAwait(false);
    }
