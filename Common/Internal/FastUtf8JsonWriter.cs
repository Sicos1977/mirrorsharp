...
internal class FastUtf8JsonWriter : IFastJsonWriter {
    ...
    public void WriteValue(StringBuilder? value) {
        ...
        WriteRawByte(Utf8.Quote);
        for (var i = 0; i < value.Length; i++) WriteUnquotedChar(value[i]);
        WriteRawByte(Utf8.Quote);
        WriteEndValue();
    }
    ...
    public void WriteValue(ArraySegment<char> value) {
        WriteStartValue();
        WriteRawByte(Utf8.Quote);
        for (var i = 0; i < value.Count; i++) WriteUnquotedChar(value.Array![value.Offset + i]);
        WriteRawByte(Utf8.Quote);
        WriteEndValue();
    }
    ...
    public void WriteValue(ImmutableArray<char> value) {
        WriteStartValue();
        WriteRawByte(Utf8.Quote);
        foreach (var @char in value) WriteUnquotedChar(@char);
        WriteRawByte(Utf8.Quote);
        WriteEndValue();
    }
    ...
    private void WriteUnquotedChar(char @char) {
        ...
        _encoder.Convert(_oneCharBuffer, 0, 1, _buffer, _position, _buffer.Length - _position, false, out _, out var bytesUsed, out _);
        _position += bytesUsed;
    }
    ...
    private void WriteEndValue() {
        ...
        else if (state == State.ObjectPropertyValue) ReplaceState(State.ObjectAfterProperty);
    }
