...
internal class FSharpSession : ILanguageSessionInternal, IFSharpSession {
    ...
    private readonly FSharpVirtualFile _sourceFile;
    ...
    public FSharpSession(
        ...
        _sourceFile = CustomFileSystem.Instance.RegisterVirtualFile(
            static s => s.EnsureTextStream(), this, "_.fs"
        );
        ...
        Checker = FSharpChecker.Create(
            ...
            false,
            false,
            FSharpOption<DocumentSource>.Some(DocumentSource.FileSystem),
            false,
            false
        );
        ...
        _projectOptions = new FSharpProjectOptions(
            "_",
            null,
            new[] { _sourceFile.Path },
            ConvertToOtherOptionsSlow(options),
    ...
    public async ValueTask<FSharpParseAndCheckResults> ParseAndCheckAsync(CancellationToken cancellationToken) {
        ...
        var tuple = await FSharpAsync.StartAsTask(
            Checker.ParseAndCheckFileInProject(_sourceFile.Path, 0, sourceText, ProjectOptions, FSharpOption<string>.None), null, cancellationToken
    ...
    public async ValueTask<Tuple<FSharpDiagnostic[], int>> CompileAsync(MemoryStream assemblyStream, CancellationToken cancellationToken) {
        ...
        try {
            ...
            var compiledAsync = Checker.Compile(args, null);
    ...
    private MemoryStream EnsureTextStream() {
        if (_sourceStream == null) {
            ...
            _sourceStream = _memoryStreamManager.GetStream("FSharpSession.SourceFile", byteCount, true);
    ...
    private FSharpList<string> ToFSharpList(ImmutableArray<string> assemblyReferencePaths) {
        ...
        for (var i = assemblyReferencePaths.Length - 1; i >= 0; i--) list = FSharpList<string>.Cons(assemblyReferencePaths[i], list);
        return list;
    }
    ...
    private string[] GetCompilerArgs() {
        ...
        for (var i = 0; i < _projectOptions.OtherOptions.Length; i++) args[2 + i] = ValidateOptionForCompilerArgs(_projectOptions.OtherOptions[i]);
    ...
    private void ConvertAndAddTo(ImmutableArray<Diagnostic>.Builder diagnostics, FSharpDiagnostic[] fsharpDiagnostics) {
        foreach (var fsharpDiagnostic in fsharpDiagnostics) diagnostics.Add(ConvertToDiagnostic(fsharpDiagnostic));
    }
